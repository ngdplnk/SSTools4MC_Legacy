#### SSTOOLS4MC LAUNCHER V1.0 FORK FOR FORGE SERVERS ####
...
# GET LATEST FORGE VERSION USED
def get_latest_forge():
    # Convert the directory path to a Path object
    path = Path('./libraries/net/minecraftforge/forge')
    ...
    latest_folder = max(directories, key=lambda f: f.stat().st_mtime)
    return latest_folder

# ENGLISH
def eng():
    # WINDOW TITLE
    window_title("Server Launcher for Minecraft Forge")
        ...
        try:    
            if rammount.lower() == "c":
                ...
                if any(char in "0123456789+-*/" for char in rammount):
                    ...
                    gbs = rammount
                gbs = int(gbs)
                if valor == "GB" and (gbs < 1 or gbs > 75):
                    cls()
                    print(f"Server Launcher for Minecraft Forge\n--------------------------------------------\n\nEnter a valid amount between 1 and 75 Gigabytes.")
                    time.sleep(1.5)
                    ram()
                elif valor == "MB" and (gbs < 1024 or gbs > 76800):
                    cls()
                    print(f"Server Launcher for Minecraft Forge\n--------------------------------------------\n\nEnter a valid amount between 1024 and 76,800 Megabytes.")
                    time.sleep(1.5)
                    ram()
                else:
                    eula = "eula.txt"
                    ...
                    forgepath = get_latest_forge().resolve()
                    comando_java = f"java -Xmx{gbs}{vjava} -Xms1G @{forgepath}/win_args.txt nogui %*"
            ...
            try:
                ...
                if confug == 1:
                    ...
                    def motdnuevo():
                        ...
                        cls()
                        motdcm = input(f'Server Launcher for Minecraft Forge\n--------------------------------------------\n\nThe current MOTD of the server is:\n\n"{motd}"\n\n(1) Edit MOTD\n(2) Go back\n(3) Return to main menu\n\nSelect one of the options= ')
                        try:
...
def esp():
    # TÍTULO DE VENTANA
    window_title("Lanzador de Servidores para Minecraft Forge")
        ...
        try:    
            if rammount.lower() == "c":
                ...
                if valor == "GB" and (gbs < 1 or gbs > 75):
                    ...
                    cls()
                    print(f"Lanzador de Servidores para Minecraft Forge\n--------------------------------------------\n\nIngresa una cantidad válida entre 1024 y 76,800 Megabytes.")
                    time.sleep(1.5)
                    ram()
                else:
                    eula = "eula.txt"
            ...
            try:
                ...
                if confug == 1:
                    ...
                    def motdnuevo():
                        ...
                        motd = colored(properties["motd"],"cyan")
                        cls()
                        motdcm = input(f'Lanzador de Servidores para Minecraft Forge\n--------------------------------------------\n\nEl MOTD actual del servidor es:\n\n"{motd}"\n\n(1) Editar el MOTD\n(2) Volver atrás\n(3) Volver al menú principal\n\nSelecciona una de las opciones= ')
                        try:
                            if any(char in "0123456789+-*/" for char in motdcm):
                                if not motdcm[0].isalpha():
                                    camb = eval(motdcm)
